...
- Optional Sources duerfen fehlschlagen, Pflichtquellen blockieren Mindmap und
  Aktivierung.
- Erkenntnis aus V1-Domain-Test: Notebook-Erstellung ist kein geeignetes
  Recovery-Mittel fuer schwache Candidate- oder Source-Coverage. Der
  Agenten-MVP sollte erst nach bestandenen Material-Gates live hochladen.
- Bei medizinischen oder anderen sensiblen Themen sollte der Notebook-Titel und
  spaetere Agenten-Kommunikation den Bildungs-Scope sichtbar halten.
...
- Wiederholter Lauf erkennt vorhandene Notebook-ID und vermeidet doppeltes
  Erstellen.
- Kein Live-Notebook-Upload wird gestartet, wenn der V1-Harness bereits bei
  `failed:candidates` oder `failed:materials` stoppt.
...
- Agenten-MVP nutzt `sources_ready` als Startsignal fuer Mindmap, Chat und
  Asset-Folgeaktionen.
