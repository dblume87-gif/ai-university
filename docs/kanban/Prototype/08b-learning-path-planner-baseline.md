...
- Erkenntnis aus V1-Domain-Test: Der Planner darf erst starten, wenn
  Material-Screening echte usable Sources oder klar markierte Gaps liefert. Der
  Agenten-MVP sollte `failed:materials` nicht als Planner-Fehler interpretieren,
  sondern als vorgelagertes Coverage-/Source-Problem.
- Wenn ein breiteres oder technischeres Ersatzthema gewaehlt wird, muss der
  Planner im resultierenden Markdown sichtbar machen, dass der Pfad ein
  Fallback-Pfad ist und nicht exakt das urspruengliche User-Ziel abdeckt.
...
- Fallback-Pfade bleiben als solche erkennbar, sobald Agenten-Goal-Expansion
  oder breitere Suchrichtung eingefuehrt wird.
...
- Agenten-MVP nutzt Planner-Output nur nach bestandenen Candidate- und
  Material-Gates.
